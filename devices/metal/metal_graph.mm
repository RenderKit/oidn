...
OIDN_NAMESPACE_BEGIN
  ...
  void MetalGraph::run(Progress& progress)
  {
    ...
    MTLBuffer_t inputBuffer = getMetalBuffer(inputProcess->getDst()->getBuffer());
    MTLBuffer_t outputBuffer = getMetalBuffer(outputTensor->getBuffer());
  ...
  MPSGraphTensor_t MetalGraph::createConv(std::shared_ptr<MetalOp>& op, MPSGraphTensor_t input)
  {
    ...
    NSData* biasData = [NSData dataWithBytes: (void*)finalBias->getData()
                                      length: finalBias->getByteSize()];
    
    auto biasDims = @[@1, @1, @1, @(size_t(finalBias->getDesc().dims[0]))];
    
    auto biasTensor =  [graph constantWithData: biasData
                                         shape: biasDims
                                      dataType: toMPSDataType(finalBias->getDesc().dataType)];
