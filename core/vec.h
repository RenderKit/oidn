...
namespace oidn {
  ...
  #define define_vec_binary_op(name, op)                                     \
    template<typename T>                                                     \
    OIDN_HOST_DEVICE_INLINE vec3f name(const vec3<T>& a, const vec3<T>& b) { \
      return vec3<T>(a.x op b.x, a.y op b.y, a.z op b.z);                    \
    }                                                                        \
    template<typename T>                                                     \
    OIDN_HOST_DEVICE_INLINE vec3f name(const vec3<T>& a, const T& b) {       \
      return vec3<T>(a.x op b, a.y op b, a.z op b);                          \
    }                                                                        \
    template<typename T>                                                     \
    OIDN_HOST_DEVICE_INLINE vec3f name(const T& a, const vec3<T>& b) {       \
