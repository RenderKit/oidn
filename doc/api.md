...
progress of filter operations and to cancel them as well. The
`oidnSetFilterProgressMonitorFunction` function registers a progress monitor
callback function (`func` argument) with payload (`userPtr` argument) for the
specified filter (`filter` argument):
