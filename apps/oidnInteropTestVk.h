...
public:
    ...
    ImageBuffer create_buffer(uint32_t width, uint32_t height, oidn::Format format) {
        return ImageBuffer(device, oidn_device, width, height, format);
    }

    ImageBuffer load_exr(std::string& path) {
        return ImageBuffer::load_exr(path, device, oidn_device, command_pool);
    }

    oidn::BufferRef create_oidn_buffer(ImageBuffer& imageBuffer) {
        return oidn_device.newBuffer(imageBuffer.get_buffer().get_ext_mem_type()->oidn,
                                     imageBuffer.get_buffer().get_ext_mem_handle(),
                                     imageBuffer.get_buffer().get_size());
    }

    void save_exr(ImageBuffer& buf, const std::string& path) {
        buf.save_exr(path, device, command_pool);
    }
